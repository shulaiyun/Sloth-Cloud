...
function compactTitle(rawName, rawIdea, locale) {
...
function normalizeProviderBaseUrl(provider) {
...
async function resolveProviderAttempt(promise, timeoutMs, timeoutError) {
    let timer = null;
    try {
...
export function detectGeneratedProjectLocale(input) {
    const signal = [
        trimText(input.projectName),
        trimText(input.idea),
...
export function buildFallbackGeneratedProjectRecipe(input, preferredName) {
...
function coerceBattleRecipe(value, locale) {
...
function coerceRecipe(raw, input, preferredName) {
...
function coerceBundle(raw, recipe) {
    ...
    if (typeof record.files === 'object' && record.files !== null && !Array.isArray(record.files)) {
        for (const [pathLike, rawValue] of Object.entries(record.files)) {
...
async function requestRecipeFromProvider(provider, input, timeoutMs, preferredName) {
...
async function requestBundleFromProvider(provider, input, recipe, timeoutMs, preferredName) {
    const apiKey = trimText(provider.apiKey);
    const baseUrl = normalizeProviderBaseUrl(provider);
    const model = trimText(provider.model);
    if (!apiKey || !baseUrl || !model) {
...
export async function planGeneratedProjectRecipe(input, preferredName, providers = []) {
    const failures = [];
    ...
    for (const provider of providers) {
        try {
            ...
            const planned = await resolveProviderAttempt(requestRecipeFromProvider(provider, input, timeoutMs, preferredName), timeoutMs, `provider=${provider.name} recipe_timeout`);
            if (planned) {
                return {
...
export async function generateProjectBundleFromModel(input, recipe, preferredName, providers = []) {
    const failures = [];
    ...
    for (const provider of prioritizedProviders) {
        try {
            const timeoutMs = timeoutFor(provider);
            const generated = await resolveProviderAttempt(requestBundleFromProvider(provider, input, recipe, timeoutMs, preferredName), timeoutMs, `provider=${provider.name} bundle_timeout`);
            if (generated) {
                return {
