...
Dependency update automation is intentionally disabled for the first public cut. This keeps the repository quiet while the public boundary, issue flow, and contribution process settle.

When maintainers are ready for automated dependency PRs, re-enable Dependabot with conservative limits and review each update against `pnpm build`, `pnpm test:ci`, and `pnpm run secret:scan`.
